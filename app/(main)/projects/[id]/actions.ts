...
interface ProjectPostRow extends RowDataPacket {
  id: number
  project_id: number
  blogger_id: string
  post_url: string
...
interface VisitorRow extends RowDataPacket {
  inf_blogid: string
  visitor_yesterday: string | Buffer
}
...
export async function updateProjectStatus(projectId: number, status: 'completed' | 'cancelled' | 'active') {
  return withConnection(async (connection) => {
    await connection.execute(
...
export async function getProjectSummary(projectId: number) {
  return withConnection(async (connection) => {
    try {
      ...
      const [projectResult] = await connection.execute<RowDataPacket[]>(`
        SELECT 
          p.start_date,
          p.end_date,
      ...
      const [statsResult] = await connection.execute<ProjectPostRow[]>(`
        SELECT 
          pp.id,
          pp.project_id,
          pp.blogger_id,
      ...
      const stats = {
        ...
        total_likes: statsResult.reduce((sum, post) => sum + (post.like_count || 0), 0),
        total_comments: statsResult.reduce((sum, post) => sum + (post.comment_count || 0), 0)
      }

      // 포스트 데이터를 ProjectPost 타입으로 변환
      const posts = statsResult.map(post => ({
        id: post.id,
      ...
      // 3. 방문자 통계 데이터 조회
      const [visitorResult] = await connection.execute<VisitorRow[]>(`
        SELECT 
          b.inf_blogid,
      ...
      // 블로거별 방문자 데이터 정리
      const visitorMap = new Map<string, Record<string, number>>()
      visitorResult.forEach((row) => {
        try {
          let visitorData: Record<string, number>
...
export async function getProjectKeywords(projectId: string) {
  return withConnection(async (connection) => {
    try {
      ...
      throw new Error('를 가져오는데 실패했습니다.')
    }
  })
} 